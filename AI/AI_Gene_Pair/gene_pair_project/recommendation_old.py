...
def generate_recommendations(
    ...
    # Statistical strength bonus: based on absolute delta effect size normalised
    if df["abs_delta_effect_size"].max() - df["abs_delta_effect_size"].min() > 0:
        stat_bonus = (df["abs_delta_effect_size"] - df["abs_delta_effect_size"].min()) / (
            df["abs_delta_effect_size"].max() - df["abs_delta_effect_size"].min()
        )
    else:
